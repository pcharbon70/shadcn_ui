...
export async function serveMotionMediaExport() {
  ...
  const files = new Map();
  for (const entry of manifest.routes.filter(e => e.request.startsWith("/examples/motion-media-capabilities") || e.status === 404)) {
    ...
    if (path.endsWith("index.html")) { files.set(path.slice(0, -10), record); files.set(path.slice(0, -11), record); }
  }
  ...
  return {
    url: "http://127.0.0.1:" + server.address().port + "/shadcn_ui/examples/motion-media-capabilities/",
