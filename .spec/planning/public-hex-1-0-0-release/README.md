...
- [x] 4.1 Section - Build `RELEASE_SHA` twice from clean checkouts.

  - [x] Use a clean controlling checkout and two new output directories; set
    `MIX_REBAR3` to the reviewed binary and activate every pinned tool version.
  - [x] Run `scripts/run-clean-candidate.mjs` twice with
    `--ref "${RELEASE_SHA}"`, then run `scripts/compare-candidate-builds.mjs`.
  - [x] Require equivalent records and actual archives, and record the final
  ...
  - [x] Audit the archive generated by each build rather than a copied or
    historical archive.

- [x] 4.2 Section - Consume the final archive in isolation.

  - [x] Run `scripts/run-clean-consumer.mjs` against the archive selected from
    the final comparison.
  - [x] Require the recorded archive checksum to equal the selected final
